...
class ImageService {
  ...
  Future<File?> pickImage({ImageSource source = ImageSource.gallery}) async {
    try {
      ...
      if (fileSize > maxSizeBytes) {
        throw Exception(
            '圖片檔案過大（${(fileSize / 1024 / 1024).toStringAsFixed(2)}MB），請選擇較小的圖片（最大 10MB）');
      }
  ...
  Future<ImageProcessResult> compressAndConvertToBase64(File imageFile) async {
    try {
      ...
      debugPrint(
          'ImageService: 壓縮後大小 ${(compressedSizeBytes / 1024).toStringAsFixed(2)}KB');
      debugPrint(
          'ImageService: Base64 前 100 個字元: ${base64String.substring(0, base64String.length > 100 ? 100 : base64String.length)}');
  ...
  /// 返回 true 如果超過限制
  Future<bool> isFileTooLarge(File file,
      {int maxSizeBytes = 10 * 1024 * 1024}) async {
