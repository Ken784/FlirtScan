...
## OCR 準確性要求（Critical）
**你必須準確識別對話文字，不得猜測或改寫：**
- 必須逐字逐句準確識別圖片中的對話文字內容
- 不得自行改寫、補充或猜測對話內容
- 如果文字模糊無法識別，請標記為無法識別，不要猜測
- originalText 必須是圖片中實際顯示的文字，不得有任何改動
...
閱讀圖片中的對話內容，區分發話者：
- **必須嚴格按照對話框的視覺位置判斷 speaker**：
  - 對話框在「右側」為 Me（使用者本人）
  - 對話框在「左側」為 Partner（對方）
  - 仔細觀察對話框的顏色、位置、對齊方式來判斷
...
4. **專屬特權 (Exclusivity)**: 評估是否建立了一種「只有我們懂」的氛圍，將其他人隔絕在外。例如：我們的祕密、我們的活動、經歷、我們才懂得的梗。
...
**重要：description 評語必須至少 40 個字，提供詳細且具體的分析說明。**
...
**重要：必須按照對話在圖片中的上下位置判斷時間順序排列 sentences。**
- sentences 陣列必須嚴格按照對話框在圖片中從上到下的順序排列
- 即使選出關鍵句，也必須保持原始對話的時間順序（由上到下）
- 不得因為分析需要而改變對話順序

1. 按照時間順序（圖片中從上到下），選出 3-8 句關鍵對話，每句提供：
   - \`originalText\`: 原始對話內容。必須是圖片中實際顯示的文字，不得改寫。如果是連續的對話，情境相關可以合併成一句話。（如：如果把"我""愛""你"分成三個對話送出，分析時應該合併成一句話"我愛你"。）
   - \`speaker\`: 發話者（"me" 或 "partner"），必須嚴格按照對話框的視覺位置判斷
